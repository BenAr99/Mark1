...
import { parseOrder, parseOrderList, parseTechnicians, TechnicianOption } from './order.mapper';
import {
  FileKind,
  Order,
  OrderStatus,
...
export class OrdersService {
  ...
  private readonly _technicians = signal<readonly TechnicianOption[]>([]);
  ...
  async loadTechnicians(): Promise<void> {
    try {
      this._technicians.set(parseTechnicians(await this.get('/technicians')));
