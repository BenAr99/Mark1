...
| `POST /orders/{id}/files`                  | добавить метаданные файла                 |
| `PUT /orders/{id}/files/{fileId}`          | загрузить содержимое файла                |
| `GET /orders/{id}/files/{fileId}/download` | скачать файл                              |
| `DELETE /orders/{id}/files/{fileId}`       | удалить файл                              |
...
фиксированные списки, ручки под них нет. При создании заказа `POST /orders` создаёт метаданные
выбранных вложений, после чего содержимое каждого файла отправляется raw-body запросом `PUT`.
Скачивание из карточки заказа выполняется через защищённую ручку `/download`.
